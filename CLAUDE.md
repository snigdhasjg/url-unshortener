...
Tracked as gaps rather than silently missing: `Hop.remoteIp` is always `null` (Vert.x's `WebClient` response doesn't expose the underlying connection without dropping to raw `HttpClient`); gzip-bomb protection is a `Range` header plus post-fetch truncation, not a true streaming cap. Native-image support was dropped on purpose: this is a long-running server (not a CLI or FaaS workload), so JDK mode is the recommended way to run it, and the reflection-config upkeep (`NativeReflectionConfig`, `quarkus.native.enable-https-url-handler`) wasn't worth paying for a startup-time/RSS win this workload doesn't need — the CI workflow, Dockerfiles, and `application.yml` no longer reference native-image at all. Still open: DNS-resolver-disable verified via AdGuard logs — best done on the target homelab hardware. Metrics (`micrometer-registry-prometheus`) intentionally omitted per `plan.md`, which marks it optional. Container supply-chain attestation is deliberately partial: CI publishes no `actions/attest` Sigstore attestation, and skips BuildKit's `provenance: true` too — that mostly restates the commit/version already in the OCI labels below, so it wasn't worth the extra build cost. `sbom: true` is kept, since a queryable SPDX SBOM per image (dependency visibility, vuln scanning) is the one of the two actually worth having. The build job's `id-token`/`attestations` permissions were dropped along with the attest step, so re-enabling it means restoring those too. Image labels carry `com.snigji.ci.id`/`com.snigji.ci.url` (run number / run URL) under the project's own reverse-DNS namespace — never `com.github.*`, which isn't ours to write into. Image tags: pushes to `main` push the semver `version` plus `latest`; PR and manual (`workflow_dispatch`) builds push `ghcr.io/<repo>:<previous_version>-<short-sha>` instead — `previous_version` (the last actually-tagged release), not the prospective `version`, since a PR/dispatch build shouldn't claim a version number it may never really get. The `org.opencontainers.image.version` label tracks whichever of the two the tag used, for the same reason.
